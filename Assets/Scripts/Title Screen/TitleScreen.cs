...
public class TitleScreen : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (!stopUpdate)
        {
            ...
            {
                if (charcustomBtn != null) charcustomBtn.interactable = true;
                if (sandboxBtn != null) sandboxBtn.interactable = true;
    ...
    private void FixedUpdate()
    {
        ...
        if (!stopUpdate)
        {
            ...
            if (characterG.alpha <= 0) characterW.SetActive(false);
        }
    }
    ...
    public void MenuFunc()
    {
        ...
        for (int i = 0; i < bars.Length; i++) bars[i].transition = true;
    }
    ...
    public void Down()
    {
        ...
        barUp = false;
        for (int i = 0; i < bars.Length; i++) bars[i].transition = false;
    ...
    public void SetSSR(int quality)
    {
        ...
        if (check < 0) check = 0;
        if (check > 6 && !disableRaytracing) check = 6;
        if (check > 4 && disableRaytracing) check = 4;
    ...
    public void SetSSAO(int quality)
    {
        ...
        if (check < 0) check = 0;
        if (check > 5 && !disableRaytracing) check = 5;
        if (check > 2 && disableRaytracing) check = 2;
    ...
    private void UpdateSettings()
    {
        ...
        if (!stopUpdate)
            if (vsyncText != null)
            {
                ...
                resPercentText.GetComponent<Text>().text =
                    (100 - PlayerPrefs.GetInt("Settings: Res Percent") * 5).ToString();
