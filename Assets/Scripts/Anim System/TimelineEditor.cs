...
public class TimelineEditor : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (audioLengthMax != 0)
        {
            ...
            if (Input.GetKey(KeyCode.LeftControl) && Input.GetKeyDown(KeyCode.S)) creator.SaveRecording();
            ...
            playbackMarker.transform.position =
                new Vector3(remap(speaker.time, viewZoomMin, viewZoomMax, 0, 1) * Screen.width,
                    playbackMarker.transform.position.y, 0);
    ...
    public void RepaintTimeline()
    {
        ...
        viewMinText.text = TimeSpan.FromSeconds(viewZoomMin).ToString().TrimEnd(new[] { '0' });
        viewMinText.text = viewMinText.text.Substring(0, Mathf.Min(11, viewMinText.text.Length));
        viewMaxText.text = TimeSpan.FromSeconds(viewZoomMax).ToString().TrimEnd(new[] { '0' });
        viewMaxText.text = viewMaxText.text.Substring(0, Mathf.Min(11, viewMaxText.text.Length));
        timeScaleText.text = "[ " + (viewZoomMax - viewZoomMin) + "s ]";
    ...
    public void AudioVideoGetData()
    {
        ...
        extraInfoText.text =
            "Length: " + audioLengthMax + "s | " + uiShowtapeManager.dataStreamedFPS + " Signal Density";
    ...
    private void CamFeedCreate()
    {
        ...
        {
            GameObject temp = Instantiate(camFeedTemplate, camFeedHolder.transform, true);
...
public class TLRecordGroup
{
    ...
    //Editor
    [HideInInspector] public GameObject currentBit;

    [HideInInspector] public bool checkedObject;
}